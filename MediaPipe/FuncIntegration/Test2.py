    ...
    while camera.isOpened():
        ...
        if results.multi_face_landmarks:
            for face_landmarks in results.multi_face_landmarks:
                ...
                # store probabilities of 7 expressions  存储7个表情的概率
                emotion = model.predict(img_pixels)[0]
                Q.append(emotion)
                
                # perform prediction averaging over the current history of previous predictions 对以前预测的当前历史执行预测平均
                results = np.array(Q).mean(axis=0)
                i = np.argmax(results)
                label = emotions[i]

                # write emotion text above rectangle  在矩形上面写上情感文字
                cv.putText(image, label, (cx_min, cy_min),cv.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

                cv.rectangle(image, (cx_min, cy_min), (cx_max, cy_max), (0, 255, 0), 2)
                #cv2.circle(image, ((cx_min+cx_max)//2, (cy_min+cy_max)//2), 100, (0, 255, 0), 2)
                                
            # check if the video writer is None 检查视频写入是否为None
            if writer is None:
                # initialize our video writer 初始化视频编写器
                h, w, c = image.shape
                fourcc = cv.VideoWriter_fourcc('D', 'I', 'V', 'X')
                writer = cv.VideoWriter(out_video_path, fourcc, 20, (w, h), True)

            # write the output frame to disk 将输出帧写入磁盘
            writer.write(image)
